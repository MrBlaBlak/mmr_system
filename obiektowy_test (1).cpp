...
//tu koñczy siê pierwsza czêœæ programu
   while(!loop)
   {
   cout<<"type 1 for team1 win, 2 for team 2 win, 3 to create your own teams 0 to leave the programm, 9 to start new teams"<<endl;
   cin>>count;  
...
//ZAPISUJEMY DANE I ZACZYNAMY NOWY MECZ
   ...
   else if(count==3)    
   {
   		...
   		{
			cout<<"Gimme Team 1 Player"<<a+1<<endl; 
			cin>>str2[a].name;			
   		}
   		cout<<endl;
   		for(unsigned a=0;a<5;a++)
   		{
			cout<<"Gimme Team 2 Player"<<a+1<<endl; 
			cin>>str3[a].name;			
   		}
   		cout<<endl;
   		for(unsigned a=0;a<5;a++)
